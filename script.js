...
// THEME TOGGLE (gantikan Dark Mode lama)
const themeBtn = document.getElementById('Toggle'); // pakai ID lama
const themes = [
  {
    name: 'Default',
    bg: '#0b1020',
    ...
    accent1: '#5f22c8', // biru
    accent2: '#06b6d4', // ungu
    accent3: '#af1660',  // merah
    accent4: '#2b0b43'  // merah
  },
  {
  name: 'Red',
  bg: '#0b1020',
  ...
  accent1: '#DC143C', // biru
  accent2: '#F7CAC9', // ungu
  accent3: '#f75252ff',  // merah
  accent4: '#430b0bff' // merah
  },
  {
  name: 'Blue',
  bg: '#0b1020',
...
themeBtn.addEventListener('click', () => {
  ...
  themeBtn.setAttribute('aria-pressed', 'true');
    ...
    // MEMBER TILT EFFECT (lightweight)
    ...
    members.forEach(card => {
      ...
      // accessible via keyboard - simple focus effect
    ...
    // GALLERY LAZY LOAD small enhancement (use loading attr)
    ...
    // small accessibility: smooth scroll for internal links
    ...
    // optional: keyboard shortcut "g" to go to gallery (demo of small JS trick)
    ...
    // tiny performance: remove heavy hover transforms on small screens
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const animateCounter = (el) => {
    ...
    const target = parseInt(originalText.replace(/\D/g, "")); // Ambil angka
    let start = 0;
    const duration = 3500; // 2 detik
    const startTime = performance.now();

    const easeOutQuad = (t) => t * (2 - t); // easing function (awal cepat, akhir lambat)
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) {
        ...
        el.textContent = "0"; // Reset saat keluar
      }
    });
  }, { threshold: 0.5 });
